...
pub fn monitor_is_selected(
    monitor: &MonitorInfo,
    target: &crate::config_v2::TargetMonitor,
) -> bool {
    ...
    match target {
        crate::config_v2::TargetMonitor::All => monitor.is_hdr_supported,
        crate::config_v2::TargetMonitor::Monitor { device_path, .. } => {
            identity_eq(identity, device_path)
        }
        crate::config_v2::TargetMonitor::NeedsConfirmation { .. } => false,
    }
}
...
pub(crate) mod tests {
    ...
    fn selection_badges_use_durable_ordinal_identity_and_reject_unavailable_identities() {
        ...
        assert!(!monitor_is_selected(
            &selected,
            &crate::config_v2::TargetMonitor::All
        ));
