...
* **Cross-Origin Manual Ordering**: GUI and tray results share actor-issued manual revisions and per-scope result snapshots. Native errors are presented through the same ordered status channel; submission/transport errors carry their scope and observed manual revision. Late failures cannot reintroduce recovered errors, late successes cannot hide newer failures, and another display's success does not clear an unrelated error.
