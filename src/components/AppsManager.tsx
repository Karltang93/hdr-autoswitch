...
export const AppsManager: React.FC<AppsManagerProps> = ({
  ...
  const handleScanInstalled = async () => {
    ...
    try {
      ...
      const added: number = await invoke('import_detected_games', { detected });
      const refreshed: AppConfig = await invoke('get_config');
      onUpdateConfig(refreshed);

      setScanMessage(
        added > 0
          ? `Nalezeno a přidáno ${added} nainstalovaných HDR her z vašeho PC (Steam/disky)!`
          : `Prohledáno: Žádné nové HDR hry nebyly nalezeny (všechny známé jsou již v seznamu).`
      );
    } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
          <button
            ...
            title="Automaticky prohledá Steam a nainstalované hry v PC"
          >
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {app.path && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20 font-mono truncate max-w-[200px]" title={app.path}>
                          Nainstalováno
                        </span>
                      )}
                    </div>
